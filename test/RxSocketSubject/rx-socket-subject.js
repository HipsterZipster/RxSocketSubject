...
describe('RxSocketSubject.create()', function () {
	...
	it('should create a Subject', function () {
		...
		expect(typeof socketSubject.observer !== 'undefined' &&
			typeof socketSubject.observable !== 'undefined').toBe(true);
	});
	...
	describe('the returned subject', function () {
		...
		describe('onNext', function () {
			it('should buffer messages if no socket exists yet', function () {
				var socket;
				var socketSubject = RxSocketSubject.create(Rx.Observable.just(''));
...
function sendClose(code, reason, wasClean) {
	...
	sockets.forEach(function (socket) {
		socket.readyState = 3;
		socket.dispatchEvent(createEvent(socket, 'close', {
...
function MockSocket(url, prototype) {
	...
	this.addEventListener =
		jasmine.createSpy('addEventListener').and.callFake(function (name, handler) {
			handlers[name] = handlers[name] || [];
			handlers[name].push(handler);
		});

	this.removeEventListener =
		jasmine.createSpy('removeEventListener').and.callFake(function (name, handler) {
			handlers[name] = handlers[name] || [];
